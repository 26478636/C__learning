...
private:
    ...
    // 调度器 做游戏的逻辑处理
    // 在每一帧的时候，做update调度......
    // 在一个场景中，如何让一个对象从A点移动到B点：在每一帧对它的坐标进行修改(即：对wmNode中的update()进行修改)
    // 然后，丢给_scheduler(调度器)，_scheduler会调用update()方法
...
public:
    ...
    // *****因为内存释放池是导演类管理的，所以向导演类申请内存释放池的方法*****
    wmScheduler *getScheduler() { return _scheduler; }
    wmAutoreleasePool *getAutoreleasePool() { return _releasepool; }
    // ------------------------------------------------------------------------------------------------
};
