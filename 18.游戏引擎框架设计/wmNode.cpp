...
void wmNode::pause()
{
    ...
    // (回调的过程)  node处理之后，需要处理schedule
    wmDirector::instance()->getScheduler()->pause(this);
}
void wmNode::resume()
{
    _pause = false;
    ...
    wmDirector::instance()->getScheduler()->resume(this);
}
// ------------------------------------------------------------------------------------------------
...
void wmNode::addChild(wmNode *node, int zorder, int tag)
{
    ...
    // node->retain(); // 引用计数+1 // 这里我们可以重写一下vector容器
    // 同理，removeChild，需要引用计数-1
    node->_zorder = zorder;
    node->_tag = tag;
